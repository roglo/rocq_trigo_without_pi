...
destruct (rngl_leb_dec 0 (rngl_cos α1))%L as [Hzc1| Hc1z]. {
  ...
  destruct (rngl_ltb_dec 0 (rngl_sin α1))%L as [Hzs1| Hs1z]. {
  ...
  destruct H as [H| H]. {
    ...
    apply (rngl_lt_0_opp Hop Hor) in Hc2z.
...
destruct (rngl_ltb_dec (rngl_sin α1) 0)%L as [Hs1z| Hzs1]. 2: {
  ...
  apply rngl_lt_le_incl in Hc1z, Hzs2, Hc2z.
  now apply rngl_sin_add_nonneg.
}
...
destruct H21 as [H21| H21]. {
  ...
  destruct (rngl_leb_dec 0 (rngl_sin α1)) as [Hzs1| Hzs1]. {
    apply rngl_leb_le in Hzs1.
    apply (rngl_mul_le_mono_nonneg_l Hop Hor (rngl_cos α1)) in Hzs12. 2: {
      now apply rngl_lt_le_incl in Hs1z.
    }
    rewrite rngl_mul_assoc in Hzs12.
    rewrite fold_rngl_squ in Hzs12.
    specialize (cos2_sin2_1 α1) as H.
    ...
    rewrite (rngl_mul_comm Hic (rngl_sin α2)) in Hzs12.
    rewrite <- rngl_sin_add in Hzs12.
    eapply (rngl_le_trans Hor); [ apply Hzs12 | ].
    rewrite <- (rngl_mul_1_r (rngl_sin α1)) at 2.
    apply (rngl_mul_le_mono_nonneg_l Hop Hor); [ easy | ].
    apply rngl_sin_bound.
  } {
    apply (rngl_leb_gt_iff Hto) in Hzs1.
    assert (Hc2z : (rngl_cos α2 ≤ 0)%L). {
    ...
    apply (rngl_mul_le_mono_nonneg_r Hop Hor _ _ (rngl_sin α2))
      in Hzs12; [ | easy ].
    do 2 rewrite <- rngl_mul_assoc in Hzs12.
    rewrite fold_rngl_squ in Hzs12.
    specialize (cos2_sin2_1 α2) as H.
    ...
    rewrite <- rngl_sin_add in Hzs12.
    eapply (rngl_le_trans Hor); [ | apply Hzs12 ].
    rewrite <- (rngl_mul_1_r (rngl_cos α2)) at 1.
  ...
  destruct (rngl_leb_dec 0 (rngl_cos α2)) as [Hzc2| Hc2z]. {
    ...
    destruct (rngl_ltb_dec 0 (rngl_sin α1)) as [Hzs1| Hc1z]. {
      apply (rngl_ltb_lt Heo) in Hzs1.
      exfalso.
      apply (rngl_nlt_ge Hor) in Hzs12.
      apply Hzs12; clear Hzs12.
  ...
  apply (rngl_le_sub_0 Hop Hor).
  destruct (rngl_leb_dec (rngl_sin α1) 0) as [Hzs1| Hzs1]. {
    apply rngl_leb_le in Hzs1.
  ...
  progress sin_cos_add_sub_right_hyp T Hzs12.
  progress sin_cos_add_sub_right_goal T.
  apply rngl_lt_le_incl in Hs1z, Hc2z, Hzs2, Hzs1.
...
apply (angle_lt_rngl_sin_add_nonneg_sin_nonneg _ α2); [ | easy ].
now left.
Qed.
...
Theorem angle_add_overflow_opp_r :
  ∀ α, α ≠ 0%A → angle_add_overflow α (- α) = true.
Proof.
intros * Htz.
...
destruct (rngl_leb_dec 0 (rngl_sin α1))%L as [Hzs1| Hs1z]. 2: {
  apply (rngl_leb_gt_iff Hto) in Hs1z.
  ...
  progress sin_cos_add_sub_straight_hyp T Hc1z.
  progress sin_cos_add_sub_straight_goal T.
  destruct (rngl_ltb_dec 0 (rngl_cos α2))%L as [Hzc2| Hc2z]. {
  ...
  progress sin_cos_add_sub_right_goal T.
  destruct (rngl_ltb_dec 0 (rngl_cos α3))%L as [Hzc3| Hc3z]. {
  ...
  move H32 before H23.
  assert (Hs12z : (0 ≤ rngl_sin (α1 + α2))%L). {
    apply rngl_lt_le_incl in Hs1z, Hc1z.
    now apply rngl_sin_add_nonneg.
  }
  assert (Hs13z : (0 ≤ rngl_sin (α1 + α3))%L). {
    apply rngl_lt_le_incl in Hs1z, Hc1z.
    now apply rngl_sin_add_nonneg.
  }
...
specialize (H1 α3 α2 Hzs3 Hzs2 H23).
destruct (rngl_leb_dec 0 (rngl_cos α3))%L as [Hzc3| Hc3z]. {
  rewrite Hzc3 in H1.
  apply rngl_leb_le in Hzc3.
  rename H1 into Hs23.
  destruct (rngl_ltb_dec (rngl_cos α2) 0)%L as [Hc2z| Hzc2]. {
...
intros H.
apply angle_add_move_0_r in H; subst α1.
