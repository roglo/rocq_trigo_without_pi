...
Theorem angle_sub_le_mono_l_lemma_1 :
  ∀ α1 α2 α3,
  (α1 ≤ α2 ≤ α3)%A
  → (0 ≤ rngl_sin (α3 - α2))%L
  → (0 ≤ rngl_sin (α3 - α1))%L
...
destruct zs1. {
  destruct zs2. {
    ...
    apply (rngl_le_opp_0 Hop Hor) in Hs31, Hs32.
  ...
  apply (rngl_le_opp_0 Hop Hor) in Hs32.
  do 2 rewrite <- angle_opp_add_distr.
  do 2 rewrite rngl_cos_opp.
...
destruct (rngl_leb_dec 0 (rngl_cos α1)) as [Hzc1| Hzc1]. {
  ...
  destruct (rngl_leb_dec 0 (rngl_cos α2)) as [Hzc2| Hzc2]. {
    ...
    progress sin_cos_opp_hyp T Hs31.
    progress sin_cos_opp_goal T.
    apply (rngl_le_opp_0 Hop Hor) in Hs31.
  ...
  apply (rngl_le_opp_0 Hop Hor) in Hs32, Hs31.
  apply (rngl_le_opp_l Hop Hor) in H23.
  destruct (rngl_leb_dec 0 (rngl_cos α3)) as [Hzc3| Hzc3]. {
    apply rngl_leb_le in Hzc3.
    change_angle_opp α3.
...
Theorem angle_sub_le_mono_l_lemma_2 :
  ∀ α1 α2 α3,
  (α1 ≤ α2 ≤ α3)%A
  → (0 < rngl_sin (α2 - α3))%L
  → (0 < rngl_sin (α1 - α3))%L
...
destruct (rngl_leb_dec 0 (rngl_cos α1)) as [Hzc1| Hzc1]. {
  ...
  progress sin_cos_opp_hyp T Hs13.
  progress sin_cos_opp_goal T.
  rewrite <- rngl_sin_sub_anticomm in Hs13, Hs23.
  do 2 rewrite (rngl_cos_sub_comm _ α3).
  move α2 before α1; move α3 before α2.
  apply (rngl_nle_gt Hor) in Hs23.
  exfalso.
  ...
  destruct (rngl_leb_dec 0 (rngl_cos α2)) as [Hzc2| Hzc2]. {
    ...
    apply rngl_lt_le_incl in Hzs1, Hzs3, Hzc1.
...
destruct s32. {
  ...
  apply rngl_leb_le.
  now apply angle_sub_le_mono_l_lemma_1.
}
