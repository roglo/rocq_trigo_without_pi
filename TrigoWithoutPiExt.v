...
Require Import Angle AngleDef Order AngleAddOverflowEquiv Distance.
...
apply rngl_leb_le in Hzs3.
apply (rngl_leb_gt_iff Hto) in Haov.
apply (rngl_nle_gt Hor) in Hzs2.
apply Hzs2; clear Hzs2.
symmetry in Hα3.
...
Theorem rngl_sin_nonneg_add_nonneg :
  ...
  (0 ≤ rngl_sin α1)%L
  → (0 ≤ rngl_sin (α1 + α2))%L
  → if angle_add_overflow α1 α2 then (rngl_sin α2 ≤ 0)%L
     else (0 ≤ rngl_sin α2)%L.
Proof.
destruct_ac.
...
destruct aov. {
  ...
  now apply (rngl_sin_add_nonneg_angle_add_not_overflow_sin_nonneg α1).
}
Qed.
...
Theorem angle_le_sub_le_add_l_lemma_1 :
  ...
  → (0 ≤ rngl_sin α3)%L
  → (0 ≤ rngl_cos α2)%L
  → (rngl_cos α3 ≤ rngl_cos (α1 - α2))%L
  → (0 ≤ rngl_sin (α1 - α2))%L
  → (0 ≤ rngl_sin (α2 + α3))%L
  → (rngl_cos (α2 + α3) ≤ rngl_cos α1)%L.
Proof.
(* thanks Geoffroy *)
destruct_ac.
destruct (Nat.eq_dec (rngl_characteristic T) 1) as [Hc1| Hc1]. {
...
destruct (rngl_leb_dec 0 (rngl_cos α3))%L as [Hzc3| Hc3z]. {
  ...
  apply (rngl_mul_le_mono_nonneg_l Hop Hor (rngl_cos α2)) in Hc123;
    [ | easy ].
  ...
  rewrite (rngl_mul_comm Hic _ (rngl_sin α2)).
...
progress sin_cos_add_sub_right_goal T.
destruct (rngl_eqb_dec (rngl_cos α2) 0) as [Hc2z| Hc2z]. {
...
destruct (rngl_leb_dec (rngl_cos α1) (rngl_cos (α2 + α3)))
    as [Hc123| Hc231]. {
  apply rngl_leb_le in Hc123.
  apply (rngl_mul_le_mono_pos_r Hop Hiq Hto _ _ (rngl_cos α2)). {
  ...
  rewrite (rngl_mul_comm Hic (rngl_cos α3)).
  rewrite <- rngl_cos_add.
  rewrite (rngl_mul_comm Hic).
  now apply (rngl_mul_le_mono_nonneg_l Hop Hor).
}
apply (rngl_leb_gt_iff Hto) in Hc231.
...
destruct (rngl_leb_dec 0 (rngl_cos α1)) as [Hzc1| Hc1z]. {
  ...
  progress sin_cos_add_sub_right_hyp T Hzs12.
  progress sin_cos_add_sub_right_goal T.
  destruct (rngl_leb_dec 0 (rngl_cos α2)) as [Hzc2| Hc2z]. {
    apply rngl_leb_le in Hzc2.
    move Hzc2 before Hzc1.
    assert (Hzc12 : (0 ≤ rngl_sin (α1 + α2))%L). {
      cbn.
      apply (rngl_le_0_add Hos Hor).
      now apply (rngl_mul_nonneg_nonneg Hos Hor).
      now apply (rngl_mul_nonneg_nonneg Hos Hor).
...
apply rngl_lt_le_incl in Hc1z, Hzs12.
...
progress sin_cos_add_sub_right_hyp T Hc1z.
progress sin_cos_add_sub_right_goal T.
destruct (rngl_leb_dec (rngl_cos α2) 0) as [Hc2z| Hzc2]. {
...
destruct (rngl_ltb_dec 0 (rngl_cos α2)) as [Hzc2| Hzc2]. {
  ...
  destruct (rngl_eqb_dec (rngl_cos α1) (rngl_cos α2)) as [Hc1c2| Hc1c2]. {
    apply (rngl_eqb_eq Heo) in Hc1c2.
    apply rngl_cos_eq in Hc1c2.
    destruct Hc1c2; subst α1. {
...
destruct (rngl_ltb_dec 0 (rngl_cos α2)) as [Hzc2| Hzc2]. {
  ...
  destruct (rngl_ltb_dec (rngl_sin (α1 - α2)) 0) as [Hs12z| Hzs12]. {
    apply (rngl_ltb_lt Heo) in Hs12z.
    eapply (rngl_lt_le_trans Hor); [ apply Hs12z | easy ].
  }
  apply (rngl_ltb_ge_iff Hto) in Hzs12.
  ...
  destruct (rngl_eqb_dec (rngl_cos α1) 0) as [Hc1z| Hc1z]. {
    apply (rngl_eqb_eq Heo) in Hc1z.
    apply eq_rngl_cos_0 in Hc1z.
    destruct Hc1z; subst α1. {
...
apply (rngl_nlt_ge Hor) in H12.
apply H12; clear H12.
destruct (rngl_eqb_dec (rngl_sin α1) 0) as [Hs1z| Hs1z]. {
  apply (rngl_eqb_eq Heo) in Hs1z.
  apply eq_rngl_sin_0 in Hs1z.
  destruct Hs1z; subst α1. {
...
Theorem angle_mul_1_l : ∀ α, (1 * α)%A = α.
Proof.
intros; cbn.
apply angle_add_0_r.
Qed.
...
Theorem angle_lim_add_add :
  ...
  angle_lim u α1
  → angle_lim v α2
  → angle_lim (λ i, (u i + v i))%A (α1 + α2).
Proof.
destruct_ac.
destruct (Nat.eq_dec (rngl_characteristic T) 1) as [Hc1| Hc1]. {
  intros * Hu Hv ε Hε.
  rewrite (rngl_characteristic_1 Hos Hc1 ε) in Hε.
...
rewrite <- rngl_mul_2_r in Hεε2.
rewrite (rngl_mul_div Hi1) in Hεε2. 2: {
  apply (rngl_2_neq_0 Hos Hc1 Hto).
}
rewrite Hεε2.
eapply (rngl_le_lt_trans Hor). {
  apply (angle_eucl_dist_triangular _ (α1 - u n)).
}
...
Theorem angle_add_not_overflow_move_add :
  ...
  → angle_add_overflow (α1 + α3) α2 = false
  → angle_add_overflow α1 (α2 + α3) = false.
Proof.
destruct_ac.
intros * H13 H132.
...
apply (angle_le_trans _ (α1 + α3))%A; [ | apply H132 ].
...
Theorem angle_add_overflow_move_add :
  ∀ α1 α2 α3,
  angle_add_overflow α2 α3 = false
  → angle_add_overflow (α1 + α2) α3 = true
  → angle_add_overflow α1 (α2 + α3) = true.
Proof.
destruct_ac.
intros * H23 H123.
...
Theorem angle_add_overflow_assoc :
  ...
  angle_add_overflow α1 α2 = false
  → angle_add_overflow α2 α3 = false
  → angle_add_overflow (α1 + α2) α3 = angle_add_overflow α1 (α2 + α3).
Proof.
intros * H12 H23.
remember (angle_add_overflow (α1 + α2) α3) as ov eqn:Hov.
symmetry in Hov |-*.
...
Theorem angle_mul_2_l : ∀ α, (2 * α = α + α)%A.
Proof.
intros; cbn.
...
Fixpoint rngl_cos_div_pow_2 α n :=
  match n with
  | 0 => rngl_cos α
  | S n' => (√((1 + rngl_cos_div_pow_2 α n') / 2))%L
  end.

Fixpoint squ_rngl_cos_div_pow_2 α n :=
  match n with
  | 0 => rngl_cos α
  | S n' => ((1 + squ_rngl_cos_div_pow_2 α n') / 2)%L
  end.
...
Theorem rngl_cos_decr :
  ∀ α1 α2, (α1 ≤ α2 ≤ π)%A → (rngl_cos α2 ≤ rngl_cos α1)%L.
Proof.
destruct_ac.
intros * (H12, H2s).
...
Theorem angle_le_dec :
  ∀ α1 α2 : angle T, {(α1 ≤ α2)%A} + {¬ (α1 ≤ α2)%A}.
Proof.
intros.
remember (α1 ≤? α2)%A as t12 eqn:Ht12.
symmetry in Ht12.
...
Theorem angle_le_antisymm : ∀ α1 α2, (α1 ≤ α2)%A → (α2 ≤ α1)%A → α1 = α2.
Proof.
destruct_ac.
intros * H12 H21.
...
destruct zs1. 2: {
  ...
  specialize (rngl_sin_nonneg_cos_le_sin_le α1 α2 Hzs1 Hzs2) as H1.
  specialize (rngl_sin_nonneg_cos_le_sin_le α2 α1 Hzs2 Hzs1) as H2.
...
destruct s2z. {
  ...
  destruct zs2. {
    ...
    destruct Hzs1; subst α1; [ easy | clear H1z ].
    apply rngl_le_neq.
    split; [ apply rngl_cos_bound | ].
    intros H2; symmetry in H2.
    apply eq_rngl_cos_opp_1 in H2; subst α2.
    now apply rngl_lt_irrefl in H12.
  }
  apply (rngl_leb_gt_iff Hto) in Hzs2.
  destruct zs1; [ | now apply (rngl_ltb_lt Heo) in H12 ].
  apply rngl_leb_le in Hzs1.
  apply (rngl_le_antisymm Hor) in Hzs1; [ | easy ].
  apply eq_rngl_sin_0 in Hzs1.
  destruct Hzs1; subst α1; [ easy | clear H1z ].
  ...
  apply eq_rngl_cos_opp_1 in Hc; subst α2.
  now apply rngl_lt_irrefl in Hzs2.
}
...
Proof.
intros * H1z H12.
destruct (angle_lt_dec α1 α2) as [Hl12| Hl12]. {
  specialize (angle_opp_lt_compat_if α1 α2 H1z Hl12) as H1.
...
Theorem rngl_sin_mul_2_l :
  ∀ α, rngl_sin (2 * α) = (2 * rngl_sin α * rngl_cos α)%L.
Proof.
destruct_ac.
intros; cbn.
...
rewrite (rngl_mul_comm Hic (rngl_cos α)).
rewrite <- rngl_mul_assoc; symmetry.
apply rngl_mul_2_l.
Qed.

Theorem rngl_cos_mul_2_l :
  ∀ α, rngl_cos (2 * α) = (rngl_cos² α - rngl_sin² α)%L.
Proof.
destruct_ac.
intros; cbn.
do 2 rewrite rngl_mul_1_r.
...
split; intros Htr. {
  destruct zst. {
    ...
    now apply (rngl_ltb_lt Heo) in Htr.
  } {
    destruct Htr as [| Htr]; [ easy | ].
    now apply (rngl_ltb_lt Heo) in Htr.
  }
